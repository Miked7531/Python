...
#
#Purpose:    Tech Academy - Python course, creating my first program.
...
def describe_game(name):
    """
        ...
        if it is not a new game, thanks the player for
...
def nice_mean(nice,mean,name):
    ...
    while stop:
        ...
        if pick == "m":
            ...
            mean = (mean+1)
            stop = False
    score(nice,mean,name) # pass the 3 varoables to the score()
...
def again(nice,mean,name):
    ...
    while stop:
        ...
        if choice == "n":
            print("\nOh, so sadm sorry to see you go!")
            stop = False
            quit()
        else:
