...
def load_gui(self):
    ...
    #buttons
    self.btn_browse1 = tk.Button(self.master,width=12,height=1,text='Browse...',command=lambda: guichall_func.addToList(self))
    self.btn_browse1.grid(row=0,column=0,padx=(25,0),pady=(50,10),sticky=W)

    self.btn_browse2 = tk.Button(self.master,width=12,height=1,text='Browse...',command=lambda: guichall_func.addToList(self))
    self.btn_browse2.grid(row=1,column=0,padx=(25,0),pady=(1,10),sticky=W)

    self.btn_check = tk.Button(self.master,width=12,height=2,text='Check for files...',command=lambda: guichall_func.addToList(self))
    self.btn_check.grid(row=2,column=0,padx=(25,0),pady=(10,10),sticky=W)

    self.btn_close = tk.Button(self.master,width=12,height=2,text='Close Program',command=lambda: guichall_func.addToList(self))
